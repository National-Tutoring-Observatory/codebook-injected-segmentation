...
class SegModel(nn.Module):
    def __init__(self, model_path='', margin=1, train_split=5, window_size=5,
                 num_moves: int = None, move_loss_weight: float = 0.5):
        ...
        hidden_size = self.topic_model.config.hidden_size

        # ----- NEW: move encoder -----
        # num_moves is number of codes in your taxonomy (size of codebook)
        if num_moves is not None:
            self.move_embedding = nn.Embedding(num_moves, hidden_size)
            # (optional) supervised auxiliary head to predict move labels
            self.move_classifier = nn.Linear(hidden_size, num_moves)
            self.move_loss_fct = nn.CrossEntropyLoss()
        else:
            self.move_embedding = None
            self.move_classifier = None
            self.move_loss_fct = None

        self.topic_loss = nn.CrossEntropyLoss()  # (you’re not actually using this currently)
        self.score_loss = MarginRankingLoss(self.margin)


    def forward(self, input_data, window_size=None):
        device, topic_loss = input_data['coheren_inputs'].device, torch.tensor(0, device=input_data['coheren_inputs'].device)
        ...
        # ---- topic-level (SimCSE) embeddings ----
        topic_context = self.topic_model(
            input_data['topic_context'], input_data['topic_context_mask']
        )[1]  # [N_ctx, H]
        topic_pos = self.topic_model(
            input_data['topic_pos'], input_data['topic_pos_mask']
        )[1]   # [N_pos, H]
        topic_neg = self.topic_model(
            input_data['topic_neg'], input_data['topic_neg_mask']
        )[1]   # [N_neg, H]

        # ---- NEW: add move embeddings (taxonomy-aware fusion) ----
        if self.move_embedding is not None:
            # each of these move tensors should be same length as corresponding topic_* batch
            move_context = self.move_embedding(input_data['topic_context_move'])  # [N_ctx, H]
            move_pos = self.move_embedding(input_data['topic_pos_move'])          # [N_pos, H]
            move_neg = self.move_embedding(input_data['topic_neg_move'])          # [N_neg, H]

            # simple early fusion: additive
            topic_context = topic_context + move_context
            topic_pos = topic_pos + move_pos
            topic_neg = topic_neg + move_neg

        # topic-level margin loss (pseudo seg training)
        ...
        for i, j, z in zip(input_data['topic_context_num'],
                           input_data['topic_pos_num'],
                           input_data['topic_neg_num']):
            topic_context_mean.append(
                torch.mean(topic_context[topic_context_count:topic_context_count + i], dim=0)
            )
            topic_pos_mean.append(
                torch.mean(topic_pos[topic_pos_count:topic_pos_count + j], dim=0)
            )
            topic_neg_mean.append(
                torch.mean(topic_neg[topic_neg_count:topic_neg_count + z], dim=0)
            )
            topic_context_count += i
            topic_pos_count += j
            topic_neg_count += z
    ...
    def infer(self, coheren_input, coheren_mask, coheren_type_id,
              topic_input=None, topic_mask=None, topic_num=None,
              topic_move_input=None):
        device = coheren_input.device
        coheren_scores, coheren_feature = self.coheren_model(
            coheren_input, coheren_mask, coheren_type_id
        )

        topic_context = self.topic_model(topic_input[0], topic_mask[0])[1]
        topic_cur = self.topic_model(topic_input[1], topic_mask[1])[1]

        if self.move_embedding is not None and topic_move_input is not None:
            move_context = self.move_embedding(topic_move_input[0])
            move_cur = self.move_embedding(topic_move_input[1])
            topic_context = topic_context + move_context
            topic_cur = topic_cur + move_cur
        ...
        topic_context_mean = pad_sequence(topic_context_mean, batch_first=True)
        topic_cur_mean = pad_sequence(topic_cur_mean, batch_first=True)
    ...
    def topic_train(self, input_data, window_size):
        ...
        # text-only SimCSE embeddings
        topic_all = self.topic_model(
            input_data['topic_train'], input_data['topic_train_mask']
        )[1]  # [N_all, H]

        # ---- NEW: fuse move embeddings ----
        if self.move_embedding is not None:
            move_all = self.move_embedding(input_data['topic_train_move'])  # [N_all, H]
            topic_all = topic_all + move_all

        true_segments, segments, neg_utts, seg_num = [], [], [], []
        count, margin_count = 0, batch_size
        ...
        for b in range(batch_size):
            ...
            pos = torch.cat(
                (topic_all[count+mid_left:count+cur_utt],
                 topic_all[count+cur_utt+1:count+mid_right]),
                dim=0
            )
            ...
            if torch.isnan(cur_loss):
                ...
                margin_count -= 1
                continue
            topic_margin_loss += cur_loss

        topic_margin_loss /= max(margin_count, 1)

        # ---- NEW: auxiliary supervision on moves ----
        aux_move_loss = torch.tensor(0.0, device=device)
        if self.move_classifier is not None and self.move_loss_fct is not None:
            # input_data['topic_train_move'] is [N_all] with move IDs
            move_logits = self.move_classifier(topic_all)  # [N_all, num_moves]
            aux_move_loss = self.move_loss_fct(
                move_logits.view(-1, move_logits.size(-1)),
                input_data['topic_train_move'].view(-1)
            )

        # final topic loss = pseudo-seg margin + λ * move classification
        topic_loss = topic_margin_loss + self.move_loss_weight * aux_move_loss
        return topic_loss
